...
- **A new endpoint needs three things, and the gate fails until it has all three:** the method, a row
  ...
  sends what the contract documents — names *and* values, since every input has one canonical value
  the wire must carry exactly — on both REST surfaces. A row with no driver is an operation nobody
  exercises.
