...
- **A contract refresh is not done when the YAML lands.** `make contract-check` (CI job
  `contract inventory`, blocking) asserts the vendored contracts against this repository, and a
  refresh that stops at the files fails it. Three things move together: the specs, the
  ...
  reason it is not implemented. A new query parameter the client never sends fails the same gate, so
  "refresh the spec and implement it later" is not a state this repository can be left in. The
  cross-repository half, `make contract-drift`, is scheduled-only and never gates a PR. See
