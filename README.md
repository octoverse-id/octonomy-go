...
> **This tree is `v2.0.0-rc.1`**, the modern line's current release, its **first release candidate**,
> and still a prerelease on purpose (`v2.0.0-alpha.1` was the first version of the line). Every
> resource group the vendored contracts publish is implemented — see
> [Implemented resources](#implemented-resources). `go get` on the `/v2` path resolves the highest
> prerelease without anyone naming a version, since Go prefers a prerelease when no stable release of
> that major exists. **The git tag is what publishes a Go module** — the GitHub release is a separate
> step that can lag it, so the releases page can show nothing for a version that already installs.
> The proxy query in [versioning.md](docs/versioning.md#release-state) is what settles which versions
> are fetchable; a version bump lands with the release PR and the tag follows it.
>
> **The candidate is the API freeze.** The suffix comes off at that freeze, not at an endpoint count —
> the five criteria are in [versioning.md](docs/versioning.md#modern-line-pre-stability), four were
> met before this cut, and the fifth is *a validated release candidate*, which is what this release
> exists to become. `rc.1` asserts that **no further breaking change is intended**; one that proves
> necessary **supersedes the candidate** (`v2.0.0-rc.2`) rather than riding a version bump, which is
> what it could still do while the line was on `-alpha.N` — `v2.0.0-alpha.2` did exactly that,
...
- [Roadmap](docs/roadmap.md) — the recipe for adding the next resource the server ships, known gaps
  inside implemented resources, and the reasoning behind decisions the issue tracker records but
  cannot explain.
